  ...
  geom_point() +
  annotate("text", x = 45, y = 17, size = 4, color = "darkred") +
  labs(title = "Bill Length vs Depth by Species", color = "Species")
                    suppressWarnings(
                        ggsave(
                            width = 3,
                            height = 3,
                            "figures/bill-length_bill-depth_scatter_annotate_text_submission_llama.png"
                        )
                    )
                    suppressWarnings(
                        ggplot_build(last_plot())
