...
ggplot(penguins, aes(x = flipper_length_mm, y = body_mass_g, color = factor(species))) +
  ...
  geom_point()
                    suppressWarnings(
                        ggsave(
                            width = 3,
                            height = 3,
                            "figures/body-mass_flipper-length_scatter_facet_species_sex_submission_gemma.png"
                        )
                    )
                    suppressWarnings(
                        ggplot_build(last_plot())
