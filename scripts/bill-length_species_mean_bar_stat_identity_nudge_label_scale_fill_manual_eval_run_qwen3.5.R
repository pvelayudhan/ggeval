...
ggplot(mean_bill, aes(x = species, y = mean_bill_length, fill = species)) +
  geom_bar(stat = "identity", fill = c("Adelie" = "#E69F00", "Chinstrap" = "#56B4E9", "Gentoo" = "#009E73")) +
  geom_text(aes(label = round(mean_bill_length, 1)), position = position_nudge(y = 0.5)) +
  theme(axis.text.x = element_text(size = 14), axis.title.x = element_text(size = 14), axis.title.y = element_text(size = 14))
                    suppressWarnings(
                        ggsave(
                            width = 3,
                            height = 3,
                            "figures/bill-length_species_mean_bar_stat_identity_nudge_label_scale_fill_manual_submission_qwen3.5.png"
                        )
                    )
                    suppressWarnings(
                        ggplot_build(last_plot())
