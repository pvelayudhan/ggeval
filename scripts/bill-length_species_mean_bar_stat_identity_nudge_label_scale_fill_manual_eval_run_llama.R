...
mean_bill$species <- as.factor(mean_bill$species)

ggplot(penguins, aes(x = species, y = mean_bill_length, fill = species)) +
  geom_bar(stat = "identity", position = position_nudge(y = 0.5)) +
  ...
  labs(x = "Species", y = "Mean Bill Length (mm)") +
  theme_classic()
                    suppressWarnings(
                        ggsave(
                            width = 3,
                            height = 3,
                            "figures/bill-length_species_mean_bar_stat_identity_nudge_label_scale_fill_manual_submission_llama.png"
                        )
                    )
                    suppressWarnings(
                        ggplot_build(last_plot())
