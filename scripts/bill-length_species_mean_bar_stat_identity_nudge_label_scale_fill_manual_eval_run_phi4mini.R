  ...
  geom_text(aes(label = round(mean_bill_length, 1)), position = position_nudge(y = 0.5), vjust = -0.5) +
  ...
  theme(legend.position = "none")
                    suppressWarnings(
                        ggsave(
                            width = 3,
                            height = 3,
                            "figures/bill-length_species_mean_bar_stat_identity_nudge_label_scale_fill_manual_submission_phi4mini.png"
                        )
                    )
                    suppressWarnings(
                        ggplot_build(last_plot())
