  ...
  geom_point() +
  annotate("segment", x = 50, y = 20, xend = 47, yend = 18, arrow = arrow(length = unit(0.3, "cm")), col = "black")
                    suppressWarnings(
                        ggsave(
                            width = 3,
                            height = 3,
                            "figures/bill-length_bill-depth_scatter_annotate_segment_submission_qwen.png"
                        )
                    )
                    suppressWarnings(
                        ggplot_build(last_plot())
