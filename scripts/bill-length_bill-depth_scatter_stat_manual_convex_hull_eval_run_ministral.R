  ...
  geom_point() +
  stat_manual(geom = "polygon", fun = function(data) data[chull(data$x, data$y), ],
              fill = NA) +
  scale_color_manual(values = c("Adelie" = "blue", "Chinstrap" = "red", "Gentoo" = "green"))
                    suppressWarnings(
                        ggsave(
                            width = 3,
                            height = 3,
                            "figures/bill-length_bill-depth_scatter_stat_manual_convex_hull_submission_ministral.png"
                        )
                    )
                    suppressWarnings(
                        ggplot_build(last_plot())
