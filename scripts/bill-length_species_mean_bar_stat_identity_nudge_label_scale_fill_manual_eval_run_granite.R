...
mean_bill <- summarise(penguins, mean_bill_length = mean(bill_length_mm, na.rm = TRUE))

ggplot(mean_bill, aes(x = species, y = mean_bill_length)) +
  geom_bar(stat = "identity", fill = c("#E69F00", "#56B4E9", "#009E73")) +
  geom_text(aes(label = round(mean_bill_length, 1)),
            nudge_y = 0.5, position = position_nudge()) +
        ...
        axis.title.y = element_text(size = 14))
                    suppressWarnings(
                        ggsave(
                            width = 3,
                            height = 3,
                            "figures/bill-length_species_mean_bar_stat_identity_nudge_label_scale_fill_manual_submission_granite.png"
                        )
                    )
                    suppressWarnings(
                        ggplot_build(last_plot())
