            ...
            built_submission <- tryCatch(
                {
                    set.seed(42)
                    ggplot(data = penguins, aes(x = flipper_length_mm, y = body_mass_g, color = sex, shape = species)) +
  geom_point()
                    suppressWarnings(
                        ggsave(
                            width = 3,
                            height = 3,
                            "figures/body-mass_flipper-length_scatter_colour_sex_shape_species_submission_qwen.png"
                        )
                    )
                    suppressWarnings(
                        ggplot_build(last_plot())
