...
ggplot(penguins, aes(x = bill_length_mm, y = bill_depth_mm, color = species, alpha = 1)) +
  geom_point() +
  stat_manual(aes(xend = mean(bill_length_mm, na.rm = TRUE), yend = mean(bill_depth_mm, na.rm = TRUE)), geom = "segment", fun = function(data) transform(data, xend = mean(data$x, na.rm = TRUE), yend = mean(data$y, na.rm = TRUE)), color = "black")
                    suppressWarnings(
                        ggsave(
                            width = 3,
                            height = 3,
                            "figures/bill-length_bill-depth_scatter_stat_manual_segment_to_centroid_submission_llama.png"
                        )
                    )
                    suppressWarnings(
                        ggplot_build(last_plot())
