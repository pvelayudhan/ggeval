            ...
            built_submission <- tryCatch(
                {
                    set.seed(42)
                    ggplot(data=penguins, aes(x=flipper_length_mm, y=body_mass_g, color=species)) + geom_point() + theme_minimal()
                    suppressWarnings(
                        ggsave(
                            width = 3,
                            height = 3,
                            "figures/flipper-length_body-mass_scatter_theme_minimal_submission_command-a-plus.png"
                        )
                    )
                    suppressWarnings(
                        ggplot_build(last_plot())
