...
ggplot(penguins, aes(x = bill_length_mm, y = bill_depth_mm, color = species)) +
  ...
  stat_manual(geom = "segment", fun = function(data) {
    transform(data, xend = mean(data$bill_length_mm, na.rm = TRUE),
              yend = mean(data$bill_depth_mm, na.rm = TRUE))
  }) +
  theme_minimal()
                    suppressWarnings(
                        ggsave(
                            width = 3,
                            height = 3,
                            "figures/bill-length_bill-depth_scatter_stat_manual_segment_to_centroid_submission_granite.png"
                        )
                    )
                    suppressWarnings(
                        ggplot_build(last_plot())
