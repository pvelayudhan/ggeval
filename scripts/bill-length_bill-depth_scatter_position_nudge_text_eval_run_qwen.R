  ...
  geom_text(aes(label = species), vjust = -1, hjust = 1, position = position_nudge(x = 0.8))
                    suppressWarnings(
                        ggsave(
                            width = 3,
                            height = 3,
                            "figures/bill-length_bill-depth_scatter_position_nudge_text_submission_qwen.png"
                        )
                    )
                    suppressWarnings(
                        ggplot_build(last_plot())
