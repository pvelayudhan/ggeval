            ...
            built_submission <- tryCatch(
                {
                    set.seed(42)
                    ggplot(island_means, aes(x = mean_flipper, y = mean_mass, label = island)) +
  geom_point(size = 4) +
  geom_label(position = position_nudge(y = 1))
                    suppressWarnings(
                        ggsave(
                            width = 3,
                            height = 3,
                            "figures/flipper-length_body-mass_scatter_position_nudge_label_submission_llama.png"
                        )
                    )
                    suppressWarnings(
                        ggplot_build(last_plot())
