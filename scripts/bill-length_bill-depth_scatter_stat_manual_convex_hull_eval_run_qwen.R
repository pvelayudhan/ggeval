  ...
  geom_point() +
  stat_manual(mapping=aes(group=1), fun=function(data) {
    chull_data <- chull(data$x, data$y)
    data[chull_data, ]
  }, geom="polygon", fun=fun, fill=NA)
                    suppressWarnings(
                        ggsave(
                            width = 3,
                            height = 3,
                            "figures/bill-length_bill-depth_scatter_stat_manual_convex_hull_submission_qwen.png"
                        )
                    )
                    suppressWarnings(
                        ggplot_build(last_plot())
