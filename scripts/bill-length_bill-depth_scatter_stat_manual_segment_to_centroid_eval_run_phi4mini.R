  ...
  stat_manual(data = penguins, mapping = aes(xend = x, yend = y), fun = function(data) transform(data, xend = mean(data$x, na.rm = TRUE), yend = mean(data$y, na.rm = TRUE)), geom = "segment", color = species)
                    suppressWarnings(
                        ggsave(
                            width = 3,
                            height = 3,
                            "figures/bill-length_bill-depth_scatter_stat_manual_segment_to_centroid_submission_phi4mini.png"
                        )
                    )
                    suppressWarnings(
                        ggplot_build(last_plot())
