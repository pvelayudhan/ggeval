  ...
  stat_manual(data = penguins, fun = function(data) data.frame(x = mean(data$flipper_length_mm, na.rm = TRUE), y = mean(data$body_mass_g, na.rm = TRUE)), geom = "point", size = 6, shape = 21, color = species)
                    suppressWarnings(
                        ggsave(
                            width = 3,
                            height = 3,
                            "figures/flipper-length_body-mass_scatter_stat_manual_centroid_submission_phi4mini.png"
                        )
                    )
                    suppressWarnings(
                        ggplot_build(last_plot())
