  ...
  geom_text(aes(label = round(mean_bill_length, 1)), position_nudge = 0.5) +
  theme(axis.title = element_text(size = 14))
                    suppressWarnings(
                        ggsave(
                            width = 3,
                            height = 3,
                            "figures/bill-length_species_mean_bar_stat_identity_nudge_label_scale_fill_manual_submission_ministral.png"
                        )
                    )
                    suppressWarnings(
                        ggplot_build(last_plot())
