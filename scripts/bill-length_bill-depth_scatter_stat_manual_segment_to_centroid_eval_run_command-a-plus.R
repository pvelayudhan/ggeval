  ...
  stat_manual(
    ...
    aes(x = bill_length_mm, y = bill_depth_mm, xend = xend, yend = yend)
  )
                    suppressWarnings(
                        ggsave(
                            width = 3,
                            height = 3,
                            "figures/bill-length_bill-depth_scatter_stat_manual_segment_to_centroid_submission_command-a-plus.png"
                        )
                    )
                    suppressWarnings(
                        ggplot_build(last_plot())
