...
ggplot(species_means, aes(x = mean_bill_length, y = mean_bill_depth, color = species)) +
  geom_point() +
  geom_text(aes(label = species), position = position_nudge(x = 0.8), color = "black") +
  theme_minimal()
                    suppressWarnings(
                        ggsave(
                            width = 3,
                            height = 3,
                            "figures/bill-length_bill-depth_scatter_position_nudge_text_submission_granite.png"
                        )
                    )
                    suppressWarnings(
                        ggplot_build(last_plot())
