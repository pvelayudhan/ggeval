            ...
            built_submission <- tryCatch(
                {
                    set.seed(42)
                    ggplot(island_means, aes(x = mean_flipper, y = mean_mass)) +

  geom_point(size = 4) +

  geom_label(data = island_means, aes(label = island), nudge_y = 1) +
  ...
  theme_minimal()
                    suppressWarnings(
                        ggsave(
                            width = 3,
                            height = 3,
                            "figures/flipper-length_body-mass_scatter_position_nudge_label_submission_phi4mini.png"
                        )
                    )
                    suppressWarnings(
                        ggplot_build(last_plot())
