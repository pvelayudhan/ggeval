  ...
  geom_point() +
  annotate(geom = "rect", x = c(40, 50), y = c(15, 19), xmin = 40, xmax = 50, ymin = 15, ymax = 19, fill = "yellow", alpha = 0.3)
                    suppressWarnings(
                        ggsave(
                            width = 3,
                            height = 3,
                            "figures/bill-length_bill-depth_scatter_annotate_rect_submission_llama.png"
                        )
                    )
                    suppressWarnings(
                        ggplot_build(last_plot())
